...
class CORO():
    ...
    def __init__(
            ...
            use_opds=False,
            use_aps=False,
            WFE_AMP=xp.ones((1000,1000)),
            WFE_OPD=xp.zeros((1000,1000)),
        ):
        ...
        self.control_rad = Nact/2 * 47/48 * 0.9
        ...
        self.WFE = poppy.ArrayOpticalElement(
            transmission=WFE_AMP, 
            opd=WFE_OPD,
            pixelscale=self.pupil_diam/(self.npix*u.pix),
            name='Coro EP WFE (pupil)',
        )
        ...
        self.inf_fun_coupling = 0.15
        inf_fun = dm.make_gaussian_inf_fun(act_spacing=self.act_spacing, sampling=sampling, coupling=self.inf_fun_coupling,  Nact=self.Nact + 2)
        self.DM1 = dm.DeformableMirror(inf_fun=inf_fun, inf_sampling=sampling, Nact=self.Nact, name='DM1 (pupil)')
        self.DM2 = dm.DeformableMirror(inf_fun=inf_fun, inf_sampling=sampling, Nact=self.Nact, name='DM2')
        ...
        self.dm1_ref = xp.zeros((self.Nact, self.Nact))
        self.dm2_ref = xp.zeros((self.Nact, self.Nact))
