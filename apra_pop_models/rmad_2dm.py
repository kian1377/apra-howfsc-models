...
class MODEL():
    ...
    def forward(self, actuators, use_vortex=True, return_ints=False, plot=False):
        ...
        if self.flip_pupil: E_PUP = xp.rot90(xp.rot90(E_PUP))
    ...
    def calc_wf(self):
        actuators = xp.concatenate([self.dm1_command[self.dm_mask], self.dm1_command[self.dm_mask]])
...
def val_and_grad(del_acts, M, actuators, E_ab, r_cond, control_mask, verbose=False, plot=False):
    ...
    x1_bar = x2_bar * M.inf_fun_fft.conjugate()
